...
module.exports.PlanoutOp = planout.Ops.Random.UniformChoice;
 ...
 */
module.exports.configure = function(options : Configuration) {
    ...
    sha1.$salt = options.salt || String(salt);
    return this;
};
 ...
 */
module.exports.create = function(name: string, choices: Choice[], options: ?Object) {
    options = options || {};
    options.name = name;
 ...
 */
module.exports.expose = function(name: string, userId: string | number, options: { log?: Boolean }) {
    if (userId == null)
        throw new Error("Missing argument. Expected: name, userId");
    if (!experiments[name])
        throw new Error("Experiment not defined: " + name);
        
    let experiment = experiments[name];
    experiment.unit = String(userId);
    experiment.salt = salt;
    
    let operator = new experiment.operator(experiment);
    let assignment = new planout.Assignment(name);
    
    planout.ExperimentSetup.toggleCompatibleHash(compatibleHash);
    let choice = operator.execute(assignment);
    let result = getValue(choice, experiment);
